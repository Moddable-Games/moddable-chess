...
export function renderSvg(options) {
  ...
  svg += renderPieces(game, rows, cols, tileSize);
...
function renderPieces(game, rows, cols, tileSize) {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const pathData = PIECE_PATHS[baseType];
      if (pathData) {
        const scale = tileSize / 45;
        out += `<g transform="translate(${x},${y}) scale(${scale.toFixed(4)})" style="fill:${fill}">${pathData}</g>\n`;
      } else {
        const cx = x + tileSize / 2;
        const cy = y + tileSize * 0.65;
        const fs = tileSize * 0.55;
        out += `<text x="${cx}" y="${cy}" text-anchor="middle" font-size="${fs}" font-weight="bold" fill="${fill}" stroke="#000" stroke-width="0.5">${baseType}</text>\n`;
      }
    }
  }
  return out;
